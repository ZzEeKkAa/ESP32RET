...
void GVRET_Comm_Handler::processIncomingByte(uint8_t in_byte) {
    ...
    case IDLE:
        if (in_byte == 0xF1) {
            ...
            // setPromiscuousMode(); //going into binary comm will set promisc.
            // mode too.
        } else {
    ...
    case GET_COMMAND:
        ...
        case PROTO_DIG_INPUTS:
            ...
            temp8 = 0; // getDigital(0) + (getDigital(1) << 1) + (getDigital(2)
                       // << 2) + (getDigital(3) << 3) + (getDigital(4) << 4) +
                       // (getDigital(5) << 5);
            transmitBuffer[transmitBufferLength++] = 0xF1;
        ...
        case PROTO_GET_CANBUS_PARAMS:
            ...
            transmitBuffer[transmitBufferLength++] =
                settings.canSettings[0].enabled +
                ((unsigned char)settings.canSettings[0].listenOnly << 4);
            transmitBuffer[transmitBufferLength++] =
                settings.canSettings[0].nomSpeed;
            transmitBuffer[transmitBufferLength++] =
                settings.canSettings[0].nomSpeed >> 8;
            transmitBuffer[transmitBufferLength++] =
                settings.canSettings[0].nomSpeed >> 16;
            transmitBuffer[transmitBufferLength++] =
                settings.canSettings[0].nomSpeed >> 24;
            transmitBuffer[transmitBufferLength++] =
                settings.canSettings[1].enabled +
                ((unsigned char)settings.canSettings[1].listenOnly << 4);
            transmitBuffer[transmitBufferLength++] =
                settings.canSettings[1].nomSpeed;
            transmitBuffer[transmitBufferLength++] =
                settings.canSettings[1].nomSpeed >> 8;
            transmitBuffer[transmitBufferLength++] =
                settings.canSettings[1].nomSpeed >> 16;
            transmitBuffer[transmitBufferLength++] =
                settings.canSettings[1].nomSpeed >> 24;
            state = IDLE;
            break;
    ...
    case SETUP_CANBUS: // todo: validate checksum
        ...
        case 3:
            ...
            if (build_int > 0) {
                if (build_int &
                    0x80000000ul) // signals that enabled and listen only status
                                  // are also being passed
                {
                    ...
                    // if not using extended status mode then just default to
                    // enabling - this was old behavior
                    settings.canSettings[0].enabled = true;
                }
    ...
    case ECHO_CAN_FRAME:
        ...
        step++;
        break;
    case SETUP_EXT_BUSES: // setup enable/listenonly/speed for SWCAN,
