...
void LAWICELHandler::handleShortCmd(char cmd) {
    ...
    case 'F':                // LAWICEL - read status bits
        Serial.print("F00"); // bit 0 = RX Fifo Full, 1 = TX Fifo Full, 2 =
                             // Error warning, 3 = Data overrun, 5= Error
                             // passive, 6 = Arb. Lost, 7 = Bus Error
        Serial.write(13);
        break;
...
void LAWICELHandler::handleLongCmd(char *buffer) {
    ...
    case 't': // transmit standard frame
        ...
        for (int data = 0; data < outFrame.length; data++) {
            outFrame.data.bytes[data] =
                Utility::parseHexString(buffer + 5 + (2 * data), 2);
        }
    ...
    case 'T': // transmit extended frame
        ...
        for (int data = 0; data < outFrame.length; data++) {
            outFrame.data.bytes[data] =
                Utility::parseHexString(buffer + 10 + (2 * data), 2);
        }
        CAN0.sendFrame(outFrame);
        if (SysSettings.lawicelAutoPoll)
            Serial.print("Z");
    ...
    case 'S':
        ...
        } else { // LAWICEL V2 - Send packet out of specified bus - S <Bus> <ID>
                 // <Data0> <Data1> <...>
            uint8_t bytes[8];
            uint32_t id;
            int numBytes = 0;
    ...
    case 's': // setup canbus baud via register writes (we can't really do
              // that...)
        // settings.CAN0Speed = 250000;
        break;
    case 'r': // send a standard RTR frame (don't really... that's so deprecated
              // its not even funny)
        break;
    case 'R':
        if (SysSettings.lawicellExtendedMode) { // Lawicel V2 - Set that we want
                                                // to receive traffic from the
                                                // given bus - R <BUSID>
            if (!strcasecmp(tokens[1], "CAN0"))
    ...
    case 'm':  // set acceptance mask - these things seem to be odd and aren't
               // actually implemented yet
    case 'M':
        if (SysSettings
                .lawicellExtendedMode) { // Lawicel V2 - Set filter mask - M
                                         // <busid> <Mask> <FilterID> <Ext?>
            int mask = strtol(tokens[2], nullptr, 16);
            int filt = strtol(tokens[3], nullptr, 16);
            if (!strcasecmp(tokens[1], "CAN0")) {
                if (!strcasecmp(tokens[4], "X"))
                    CAN0.setRXFilter(0, filt, mask, true);
                else
    ...
    case 'H':
        if (SysSettings
                .lawicellExtendedMode) { // Lawicel V2 - Halt reception of
                                         // traffic from given bus - H <busid>
            if (!strcasecmp(tokens[1], "CAN0"))
        ...
        break;
    case 'U':  // set uart speed. We just ignore this. You can't set a baud rate
               // on a USB CDC port
    ...
    case 'Z':  // Turn timestamp off/on
        ...
        break;
    case 'Q':  // turn auto start up on/off - probably don't need to actually
               // implement this at the moment.
        break; // no action yet or maybe ever
    case 'C':  // Lawicel V2 - configure one of the buses - C <busid> <speed>
               // <any additional needed params>
        if (SysSettings.lawicellExtendedMode) {
            // at least two parameters separated by spaces. First BUS ID (CAN0,
            // CAN1, SWCAN, etc) then speed (or more params separated by #'s)
