...
size_t CommBuffer::numAvailableBytes() { return transmitBufferLength; }

void CommBuffer::clearBufferedBytes() { transmitBufferLength = 0; }

uint8_t *CommBuffer::getBufferedBytes() { return transmitBuffer; }
...
void CommBuffer::sendFrameToBuffer(CAN_FRAME &frame, int whichBus) {
    ...
    if (settings.useBinarySerialComm) {
        ...
        writtenBytes = sprintf((char *)&transmitBuffer[transmitBufferLength],
                               "%d - %x", micros(), frame.id);
