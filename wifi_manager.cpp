...
WiFiManager::WiFiManager() { lastBroadcast = 0; }
...
void WiFiManager::setup() {
    ...
    if (settings.wifiMode == 2) // BE an AP
    {
        WiFi.mode(WIFI_AP);
        WiFi.setSleep(true);
        WiFi.softAP((const char *)settings.SSID,
                    (const char *)settings.WPA2Key);
        if (SysSettings.fancyLED) {
...
void WiFiManager::loop() {
    ...
    if (settings.wifiMode > 0) {
        if (!SysSettings.isWifiConnected) {
            ...
            if (needServerInit) {
                ...
                // MDNS.begin wants the name we will register as without the
                // .local on the end. That's added automatically.
                if (!MDNS.begin(deviceName))
                    Serial.println("Error setting up MDNS responder!");
                MDNS.addService("telnet", "tcp", 23); // Add service to MDNS-SD
                MDNS.addService("ELM327", "tcp",
                                1000); // Add service to MDNS-SD
                ...
                ArduinoOTA
                    .onStart([]() {
                        String type;
                        if (SysSettings.fancyLED) {
                            leds[SysSettings.LED_CONNECTION_STATUS] =
                                CRGB::Purple;
                            FastLED.show();
                        }
                ...
                if (wifiServer.hasClient()) {
                    for (i = 0; i < MAX_CLIENTS; i++) {
                        if (!SysSettings.clientNodes[i] ||
                            ...
                            if (!SysSettings.clientNodes[i])
                                Serial.println(
                                    "Couldn't accept client connection!");
                            else {
                                Serial.print("New client: ");
                                Serial.print(i);
                                Serial.print(' ');
                                Serial.println(
                                    SysSettings.clientNodes[i].remoteIP());
                                if (SysSettings.fancyLED) {
                                    leds[SysSettings.LED_CONNECTION_STATUS] =
                                        CRGB::Blue;
                                    FastLED.show();
                    ...
                    for (i = 0; i < MAX_CLIENTS; i++) {
                        if (!SysSettings.wifiOBDClients[i] ||
                            ...
                            if (!SysSettings.wifiOBDClients[i])
                                ...
                                Serial.print(' ');
                                Serial.println(
                                    SysSettings.wifiOBDClients[i].remoteIP());
                            }
                        }
                    }
                    if (i >= MAX_CLIENTS) {
                ...
                for (i = 0; i < MAX_CLIENTS; i++) {
                    if (SysSettings.clientNodes[i] &&
                        ...
                        if (SysSettings.clientNodes[i].available()) {
                            // get data from the telnet client and push it to
                            // input processing
                            while (SysSettings.clientNodes[i].available()) {
                                uint8_t inByt;
                                inByt = SysSettings.clientNodes[i].read();
                                SysSettings.isWifiActive = true;
                                // Serial.write(inByt); //echo to serial - just
                                // for debugging. Don't leave this on!
    ...
    if (SysSettings.isWifiConnected &&
        ((micros() - lastBroadcast) >
         1000000ul)) // every second send out a broadcast ping
    {
...
void WiFiManager::attemptOTAUpdate() {
    ...
    // TODO: figure out where github stores files in releases and/or how and
    ...
    if (wifiClient.connect(otaHost, port)) {
        ...
        while (wifiClient.available()) {
            ...
            // if the the line is empty,this is end of headers break the while
            // and feed the remaining `client` to the Update.writeStream();
            ...
            if (line.startsWith("HTTP/1.1")) {
                if (line.indexOf("200") < 0) {
                    Serial.println("FAIL...Got a non 200 status code from "
                                   "server. Exiting OTA Update.");
                    break;
                }
            }
            ...
            if (line.startsWith("Content-Length: ")) {
                ...
                Serial.println("              ...Server indicates " +
                               String(contentLength) + " byte file size\n");
            }

            if (line.startsWith("Content-Type: ")) {
                String contentType = getHeaderValue(line, "Content-Type: ");
                Serial.println("\n              ...Server indicates correct " +
                               contentType + " payload.\n");
        ...
        Serial.println("Connection to " + String(host) +
                       " failed. Please check your setup!");
    }

    // Check what is the contentLength and if content type is
    // `application/octet-stream`
    // Serial.println("File length: " + String(contentLength) + ", Valid Content
    // Type flag:" + String(isValidContentType));

    // check contentLength and content type
    if (contentLength &&
        isValidContentType) // Check if there is enough to OTA Update
    {
        bool canBegin = Update.begin(contentLength);
        if (canBegin) {
            Serial.println(
                "There is sufficient space to update. Beginning update. \n");
            ...
            if (written == contentLength) {
                ...
                Serial.println(
                    "\n********** FAILED - Wrote:" + String(written) + " of " +
                    String(contentLength) + ". Try again later. ********\n\n");
                return;
            }
            ...
            if (Update.end()) {
                ...
                if (Update.isFinished()) {
                    ...
                    Serial.println(
                        "FAILED...update not finished? Something went wrong!");
